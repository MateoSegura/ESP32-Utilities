...
{
  MCP2518FD *canDriver = (MCP2518FD *)pData;
  while (1)
  {
...
MCP2518FD::MCP2518FD(const uint8_t inCS,    // CS input of MCP2517FD
...
uint32_t MCP2518FD::begin(const MCP2518FDSettings &inSettings,
                          ...
                          const ACAN2517FDFilters &inFilters)
{
  ...
  if (errorCode == 0)
  {
    ...
    {
    case MCP2518FDSettings::OSC_4MHz:
    case MCP2518FDSettings::OSC_20MHz:
    case MCP2518FDSettings::OSC_40MHz:
      break;
    case MCP2518FDSettings::OSC_4MHz_DIVIDED_BY_2:
    case MCP2518FDSettings::OSC_20MHz_DIVIDED_BY_2:
    case MCP2518FDSettings::OSC_40MHz_DIVIDED_BY_2:
      osc = 1 << 4; // Divide by 2
      break;
    case MCP2518FDSettings::OSC_4MHz10xPLL_DIVIDED_BY_2:
      pll = 1;      // Enable 10x PLL
      osc = 1 << 4; // Divide by 2
      break;
    case MCP2518FDSettings::OSC_4MHz10xPLL:
      pll = 1; // Enable 10x PLL
      break;
    }
    osc |= pll;
    if (inSettings.mCLKOPin != MCP2518FDSettings::SOF)
    {
  ...
  if (errorCode == 0)
  {
    ...
    mTXQBufferPayload = MCP2518FDSettings::objectSizeForPayload(inSettings.mControllerTXQBufferPayload);
    ...
    mReceiveFIFOPayload = MCP2518FDSettings::objectSizeForPayload(inSettings.mControllerReceiveFIFOPayload);
    ...
    mTransmitFIFOPayload = MCP2518FDSettings::objectSizeForPayload(inSettings.mControllerTransmitFIFOPayload);
...
bool MCP2518FD::tryToSend(const CANFDMessage &inMessage)
{
...
bool MCP2518FD::enterInTransmitBuffer(const CANFDMessage &inMessage)
{
  bool result;
...
void MCP2518FD::appendInControllerTxFIFO(const CANFDMessage &inMessage)
{
...
bool MCP2518FD::sendViaTXQ(const CANFDMessage &inMessage)
{
  bool ok = mUsesTXQ;
  if (ok)
  {
...
bool MCP2518FD::receive(CANFDMessage &outMessage)
{
  mSPI.beginTransaction(mSPISettings);
#ifdef ARDUINO_ARCH_ESP32
  taskDISABLE_INTERRUPTS();
...
bool MCP2518FD::dispatchReceivedMessage(const tFilterMatchCallBack inFilterMatchCallBack)
{
...
void MCP2518FD::isr_poll_core(void)
{
  mSPI.beginTransaction(mSPISettings);
#ifdef ARDUINO_ARCH_ESP32
  taskDISABLE_INTERRUPTS();
...
void MCP2518FD::transmitInterrupt(void)
...
void MCP2518FD::receiveInterrupt(void)
{
...
void MCP2518FD::deassertCS(void)
{
...
void MCP2518FD::writeRegister32Assume_SPI_transaction(const uint16_t inRegisterAddress,
...
void MCP2518FD::writeRegister8Assume_SPI_transaction(const uint16_t inRegisterAddress,
...
uint32_t MCP2518FD::readRegister32Assume_SPI_transaction(const uint16_t inRegisterAddress)
{
  //--- Read word register via 6-byte buffer (speed enhancement, thanks to thomasfla)
  uint8_t buffer[6] = {0};
  //--- Enter command
...
uint16_t MCP2518FD::readRegister16Assume_SPI_transaction(const uint16_t inRegisterAddress)
{
...
uint8_t MCP2518FD::readRegister8Assume_SPI_transaction(const uint16_t inRegisterAddress)
{
...
void MCP2518FD::writeRegister8(const uint16_t inRegisterAddress, const uint8_t inValue)
{
  mSPI.beginTransaction(mSPISettings);
#ifdef ARDUINO_ARCH_ESP32
  taskDISABLE_INTERRUPTS();
...
uint8_t MCP2518FD::readRegister8(const uint16_t inRegisterAddress)
{
  mSPI.beginTransaction(mSPISettings);
#ifdef ARDUINO_ARCH_ESP32
  taskDISABLE_INTERRUPTS();
...
uint16_t MCP2518FD::readRegister16(const uint16_t inRegisterAddress)
{
  mSPI.beginTransaction(mSPISettings);
#ifdef ARDUINO_ARCH_ESP32
  taskDISABLE_INTERRUPTS();
...
void MCP2518FD::writeRegister32(const uint16_t inRegisterAddress, const uint32_t inValue)
{
  mSPI.beginTransaction(mSPISettings);
#ifdef ARDUINO_ARCH_ESP32
  taskDISABLE_INTERRUPTS();
...
uint32_t MCP2518FD::readRegister32(const uint16_t inRegisterAddress)
{
  mSPI.beginTransaction(mSPISettings);
#ifdef ARDUINO_ARCH_ESP32
  taskDISABLE_INTERRUPTS();
...
MCP2518FD::OperationMode MCP2518FD::currentOperationMode(void)
{
  const uint8_t mode = readRegister8(CON_REGISTER + 2) >> 5;
  return MCP2518FD::OperationMode(mode);
}

//······················································································································

bool MCP2518FD::recoverFromRestrictedOperationMode(void)
{
  bool recoveryDone = false;
  if (currentOperationMode() == MCP2518FD::RestrictedOperation)
...
void MCP2518FD::reset2517FD(void)
{
...
uint32_t MCP2518FD::errorCounters(void)
{
...
uint32_t MCP2518FD::diagInfos(const int inIndex)
