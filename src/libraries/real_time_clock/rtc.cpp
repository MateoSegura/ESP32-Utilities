...
ESP_ERROR RealTimeClock::begin(RTC_TYPE rtc_ic, TwoWire &port)
{
    ...
    case RV3028_IC:
        ...
        {
            if (rv3028.updateTime() == false) // Updates the time variables from RTC
            {
                err.on_error = true;
                err.debug_message = "Real time clock failed to update time";
            }
            ...
            {
                rtc_class_time.year = rv3028.getYear();
                rtc_class_time.month = rv3028.getMonth();
                rtc_class_time.day = rv3028.getDate();
                rtc_class_time.hours = rv3028.getHours();
                rtc_class_time.minutes = rv3028.getMinutes();
                rtc_class_time.seconds = rv3028.getSeconds();
            }
        }
        break;
    ...
    default:
        err.on_error = true;
        err.debug_message = "Unknown chip";
        break;
    }
...
ESP_ERROR RealTimeClock::setToCompilerTime()
{
    ...
    case RV3028_IC:
        if (rv3028.setToCompilerTime() == false)
        {
            err.on_error = true;
            err.debug_message = "Could not set to compiler time";
        }
...
ESP_ERROR RealTimeClock::updateTime(DateTime new_time)
{
    ...
    case RV3028_IC:
        if (rv3028.setTime(new_time.seconds,
                           new_time.minutes,
                           new_time.hours,
                           2, // weekday
                           new_time.day,
